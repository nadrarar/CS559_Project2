...
//	glDisable(GL_CULL_FACE);
	...
	this->RenderIntoFrameBuffer();
	this->UseFramebufferToDrawSomething();
	glutSwapBuffers();
}
void MainWindow::RenderIntoFrameBuffer(){
	fbo.Bind();
	
	float theta = ((float) this->x) / ((float) fbo.size.x) * 360.0f - 180.0f;
	float rho = ((float) this->y) / ((float) fbo.size.y) * 360.0f - 180.0f;

	this->ClearWindow();
	glClearColor(0.1f, 0.1f, 0.1f, 0.0f);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	this->DrawSkybox(fbo.size.x,fbo.size.y);	
	this->DisplayWindowName("Main Window",fbo.size.x,fbo.size.y);
	...
	glm::mat4 projection_matrix = glm::perspective(20.0f, ((float) fbo.size.x) / ((float) fbo.size.y), 1.0f, 10.0f);
...
void MainWindow::UseFramebufferToDrawSomething(){
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluPerspective(15, double(this->width) / double(this->height), 1, 10);
...
bool MainWindow::PostGLInitialize(void * data)
{
	...
	if (!fbo.Initialize(glm::ivec2(this->width*4, this->height*4), 2, true))
		{
...
void MainWindow::DrawSkybox(int width, int height)
{
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	gluPerspective(20.0, double(width) / double(height), 1, 10);
